...
1> erl_ts:init(),
   {ok, Parser} = erl_ts:ts_parser_new(),
   {ok, Lang} = erl_ts:tree_sitter_erlang(),
   true = erl_ts:ts_parser_set_language(Parser, Lang),
   SC = "a_qt_function({A, B}) -> {A+A, B*B}.\nb_qt_function(A, B) -> #{a => A, b => B}.",
   Tree = erl_ts:ts_parser_parse_string(Parser, undefined, SC),
   RootNode = erl_ts:ts_tree_root_node(Tree),
   RootNodeStr = erl_ts:ts_node_string(RootNode).
...
ok
3> {Query, _, _} = erl_ts:ts_query_new(Lang, "(function_clause name: (atom) @fun_name)").
{#Ref<0.3646334886.4285399041.132320>,3418416122,error_none}
4> Captures = erl_ts:ts_query_capture(RootNode, Query).
[{"fun_name",#Ref<0.733339053.444203009.70844>},
 {"fun_name",#Ref<0.733339053.444203009.70843>}]
5> [{C, erl_ts:ts_node_text(N, SC)} || {C, N} <- Captures].
